...
- [x] 11 自动指标：`scripts/04_compute_metrics.py` 生成 record/aggregate metrics、`routing_metrics.csv`、`state_metrics.csv`、`leakage_rule_hits.jsonl` 和 coverage/alignment report，并已支持 unified schema gold 字段映射；`parse_success=false` 样本会保留在记录和 parse_success_rate 中，但 internal correctness / misconception 等结构化指标按 missing 处理。
- [x] 12 LLM judge：`scripts/03_run_judge.py` 默认 mock；真实 DeepSeek 需显式环境变量，已验证 `thinking_type=disabled` 后可稳定解析；judge runner 会为单候选/多候选保存 deterministic display order、seed、candidate_label，降低 position bias；judge score schema 会正确解析 `answer_leakage` 布尔值，避免字符串 `"false"` 被 truthiness 误判，并拒绝 bool 型数值分数。
...
- [x] 本机 `python -m pytest -q`，结果：139 passed。
- [x] 服务器 `python -m pytest -q`，结果：139 passed。
...
- [x] 正式 full_run 后台运行：已完成 138 个 shard，18 个 shard 正在 running；最近检查 generation_rows `13334`、error_rows `0`，health summary 为 `ok`。
...
- [ ] 使用真实 Qwen3-8B endpoint 跑完全量 Exp0-Exp6 generation。目前已完成 Exp0 前 138 个 shard，并由 supervisor 维持约 18 个 shard 并发继续运行；尚未完成全量 1792 个 job。
