...
- [x] Shard supervisor：`python scripts/11_plan_or_run_shards.py supervise --plan outputs/full_run/shard_plan.json --target-running 16 --interval-seconds 120 --log-path outputs/full_run/logs/shards/supervisor_compact.jsonl`，服务器 PID `2469692`。
...
- [x] 正式 full_run 后台运行：已完成 16 个 shard，16 个 shard 正在 running；最近检查 generation_rows `2388`、error_rows `0`，近期吞吐约 `74.04` rows/min，粗略 ETA 约 `45.46` hours，GPU2/GPU3 均 100% utilization，health summary 为 `ok`。
...
- [ ] 使用真实 Qwen3-8B endpoint 跑完全量 Exp0-Exp6 generation。目前已完成 Exp0 前 16 个 shard，并由 supervisor 维持 16 个 shard 并发继续运行；尚未完成全量 1792 个 job。
