...
- [x] 本机 `python -m pytest -q`，结果：183 passed。
- [x] 服务器 `python -m pytest -q`，结果：182 passed。
...
- [x] 正式 full_run 后台运行：已完成 231 个 shard，18 个 shard 正在 running；最近 maintainer 补齐 1 个 shard：`exp0_problem_diagnosis_shard248-of-256`；最新 generation_rows `20603`、error_rows `0`，estimated_records `294053`，completion_fraction `0.07006560041897208`；recent_rows_per_minute `71.41894073786825`，ETA 约 `63.81360396715447` 小时；health summary 为 `ok`。
