...
- [x] 14 统计、表格和图：`scripts/05_make_tables.py`、`scripts/06_make_figures.py`；tables/significance 使用合并后的 `final_leakage`、`judge_leakage`、`leakage_conflict`，并能把 Exp6 的 `base__noise...__budget...` 方法变体纳入 robustness 表。
...
- [x] 本机 `python -m pytest -q`，结果：150 passed。
- [x] 服务器 `python -m pytest -q`，结果：150 passed。
...
- [x] 正式 full_run 后台运行：已完成 158 个 shard，18 个 shard 正在 running；最近检查 generation_rows `15091`、error_rows `0`，health summary 为 `ok`。
...
- [ ] 使用真实 Qwen3-8B endpoint 跑完全量 Exp0-Exp6 generation。目前已完成 Exp0 前 158 个 shard，并由 supervisor 维持约 18 个 shard 并发继续运行；尚未完成全量 1792 个 job。
