...
本仓库已经具备 Task Card 01-16 的可运行代码骨架、mock/dry-run 端到端链路，以及服务器上的 Qwen3-8B vLLM live smoke 链路。MathDial、Bridge 与 MaE Math Misconceptions 三个真实数据集均已下载、传输到服务器并完成 schema build。Exp0-Exp6 已完成真实 Qwen live smoke（limit=1）与 downstream metrics/tables/figures/artifact smoke；真实 DeepSeek judge 已对 live smoke 123 条 generation 完成评分。runner 已支持 sample-level sharding，用于拆分全量正式实验，并已支持 `maintain --target-running N` 与 `supervise` 可恢复补齐后台并发；新建 shard plan 默认绑定当前 git commit，launch/maintain/supervise 会在 commit mismatch 或 dirty worktree 时拒绝启动，除非显式使用 smoke-only override。PRISM runtime 现在会从 `configs/default.yaml` 注入 risk weights、thresholds、budget 与 commit threshold；Exp5 ablation 会真实禁用或替换对应模块/风险信号；Exp6 robustness 会展开 noisy-agent probability × token budget 变体，并把 deterministic noisy injection 参数写入 raw log。全量完成后可用 `scripts/12_finalize_full_run.py` gated finalization 生成 metrics/tables/figures/human audit sample/paper artifacts；finalization manifest/stdout 会输出 completed/total/can_finalize/planned_steps、命令 invocation、reproducibility metadata、unresolved error_rows 与 raw_error_rows，并会汇总 shard plan git freeze、每个 generation shard manifest 记录的 git commit 分布、缺失 manifest、缺失 commit 和 dirty manifest 数量；formal finalization 现在要求 clean git-frozen shard plan，并默认拒绝 mixed generation commits，除非显式传入 `--allow-mixed-generation-commits`，用于避免长跑期间代码持续演进污染论文级正式结果；同时会把每个后处理 step 的 stdout/stderr 保存到 `outputs/full_run/logs/finalization`；formal finalization 会拒绝 incomplete shards、unresolved generation error rows、缺失 judge source，以及 smoke-only flags，非 dry-run 时某一步失败会 fail-fast 并将后续步骤标记为 skipped；paper artifact exporter 会检查 `outputs/full_run/*` run-local 产物；human audit sampler 会在正式模式下要求 metrics、generation logs、judge scores 和 tables 全部存在，并用 generation logs 回填 blind CSV 所需 response/context 字段；finalization 支持人工标注完成后用 `--run-human-agreement` 在 paper artifact 前生成 run-local agreement report，并显式传入同目录 `preference_mapping.json`；formal `--run-human-agreement` 会在入口处要求 `human_audit_labeled.csv` 已存在，避免前置后处理跑完后才失败；table builder 会从 deduplicated record metrics 自动导出 Table 1-6 及 significance JSON；metrics aggregate 现在会对 judge 恢复/重跑行按 dataset/sample/method 去重，优先保留有效 parsed_score，并在 coverage report 中记录 raw/duplicate/replaced counts；paper artifact exporter 可用 full-run shard plan 生成覆盖 Exp0-Exp6 的 experiment manifest，并记录 datasets、split、methods、job count、estimated records 和输出路径；reproducibility checklist 会记录 seed/config/model/generation、GPU、judge metadata、data/log paths、git 和 package versions；`scripts/11_plan_or_run_shards.py progress` 可从 supervisor log 输出完成率、近期吞吐、ETA 与 health summary，并可识别 running 高于或低于 supervisor target、zombie PID 和 unresolved generation errors。当前全量 Exp0-Exp6 长跑已经开始运行并由服务器 supervisor 接管，但 dry-run 审计发现已完成 shard 覆盖多个 generation commit，因此这批运行更适合作为稳定性/恢复性验证；若要作为论文正式结果，需要在稳定代码 commit 上重跑，或对 mixed commit 进行显式 provenance override；正式 200 条 blind human audit 也尚未执行。
...
- [x] 08 Baseline 方法：method registry 覆盖 B0-B5 与实验变体；`prism_tutor/baselines/` 提供 Single Tutor、Fixed 2、Fixed 4、Debate、Generic Sparse、Difficulty Routing 与 Oracle Routing planner；runner live baseline 路径会记录 `baseline_plan`，Generic Sparse 与 Difficulty Routing 有禁止读取教育风险字段的 fairness tests；Oracle Routing 已支持 list-valued gold labels。
...
- [x] 11 自动指标：`scripts/04_compute_metrics.py` 生成 record/aggregate metrics、`routing_metrics.csv`、`state_metrics.csv`、`leakage_rule_hits.jsonl`、`leakage_metrics.csv` 和 coverage/alignment report，并已支持 unified schema gold 字段映射；`parse_success=false` 样本会保留在记录和 parse_success_rate 中，但 internal correctness / misconception 等结构化指标按 missing 处理；恢复补跑产生的 duplicate generation rows 会按 dataset/sample/split/method 去重，优先使用最新 success，coverage report 记录 raw/duplicate/replaced counts；judge row 只有在 `parsed_score` 有效且无 `error` 时才计入 judge leakage coverage，失败 judge 不会被当作 non-leakage；恢复补跑产生的 duplicate judge rows 会按 dataset/sample/method 去重，优先使用有效 parsed_score，coverage report 记录 raw/duplicate/replaced counts；metric alignment report 会记录无法匹配 generation 的 orphan judge rows。
- [x] 12 LLM judge：`scripts/03_run_judge.py` 默认 mock；真实 DeepSeek 需显式环境变量，已验证 `thinking_type=disabled` 后可稳定解析；judge runner 会为单候选/多候选保存 deterministic display order、seed、唯一 candidate_label，降低 position bias，并在评分前对恢复补跑产生的 duplicate generation rows 去重；judge score schema 会正确解析 `answer_leakage` 布尔值，避免字符串 `"false"` 被 truthiness 误判，并拒绝 bool 型数值分数；metrics/finalization 支持合并 rule leakage 与 judge leakage，输出 `judge_leakage`、`final_leakage` 和 `leakage_conflict`；run-level judge metadata 汇总 actual_models、parsed_count、error_count、output_rows、raw_response_count 和 generation_deduplication；formal finalization 的 judge step 默认传 `--require-real`，只有显式 `--allow-mock-judge` 才允许 smoke mock judge。
...
- [x] Human agreement finalization gate：`scripts/12_finalize_full_run.py --run-human-agreement` 会在 `paper_artifacts` 前运行 `scripts/08_human_agreement.py`，并使用 full-run 本地 `human_audit_labeled.csv` / `human_agreement_report.json` / `preference_mapping.json` 路径；formal 模式会在入口处拒绝缺少 `human_audit_labeled.csv` 的 agreement run。
