...
- [x] 服务器 `python -m pytest -q`，结果：164 passed。
...
- [x] 正式 full_run 后台运行：已完成 176 个 shard，19 个 shard 正在 running；最近检查 generation_rows `17099`、error_rows `0`，estimated_records `294053`，completion_fraction `0.05814938123399523`；health summary 为 `warn`，原因是 running 高于 target 18，当前应等待运行中 job 回落后再 maintain。
...
- [ ] 使用真实 Qwen3-8B endpoint 跑完全量 Exp0-Exp6 generation。目前已完成 Exp0 前 176 个 shard，并由 supervisor 维持运行；当前 running 高于 target 18，尚未完成全量 1792 个 job。
