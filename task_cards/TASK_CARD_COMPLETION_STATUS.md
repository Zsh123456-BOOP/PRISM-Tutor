...
- [x] 正式 full_run 后台运行：已完成 36 个 shard，18 个 shard 正在 running；最近检查 generation_rows `4390`、error_rows `0`，近期吞吐约 `86.77` rows/min，粗略 ETA 约 `38.42` hours，GPU2/GPU3 均 100% utilization，health summary 为 `ok`。
...
- [ ] 使用真实 Qwen3-8B endpoint 跑完全量 Exp0-Exp6 generation。目前已完成 Exp0 前 36 个 shard，并由 supervisor 维持 18 个 shard 并发继续运行；尚未完成全量 1792 个 job。
