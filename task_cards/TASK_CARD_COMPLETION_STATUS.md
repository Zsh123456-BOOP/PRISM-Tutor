...
- [x] 本机 `python -m pytest -q`，结果：177 passed。
...
- [x] 正式 full_run 后台运行：已完成 195 个 shard，18 个 shard 正在 running；最近检查 generation_rows `18577`、error_rows `0`，estimated_records `294053`，completion_fraction `0.06317568601578627`；recent_rows_per_minute `70.2406122993937`，ETA 约 `65.36484401782894` 小时；health summary 为 `ok`。
