...
from .correctness import evaluate_solver_correctness
...
def _judge_numeric(judge_row: dict[str, Any], field: str) -> float | None:
...
def compute_record_metrics(
    ...
    if parsed_success:
        correctness_metrics = evaluate_solver_correctness(record, gold)
        ...
        correctness_metrics = {
            ...
            "solver_correctness_reason": "parse_failed",
    ...
    output = {
        ...
        **evaluate_state_metrics(record, gold),
        ...
        output.update(
            {
                ...
                "tutor_math_correctness": _judge_numeric(judge_row, "mathematical_correctness"),
                "next_turn_feedback_quality": _judge_numeric(judge_row, "next_turn_feedback_quality"),
                "student_state_correctness": _judge_numeric(judge_row, "student_state_correctness"),
        ...
        output.update(
            {
                ...
                "tutor_math_correctness": None,
                "next_turn_feedback_quality": None,
                "student_state_correctness": None,
...
def _aggregate_group(dataset: str, method: str, rows: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "student_state_correctness_mean": _mean_present(rows, "student_state_correctness"),
        ...
        "external_state_coverage": _mean_present(rows, "external_state_coverage"),
...
def _missing_gold_counts(rows: list[dict[str, Any]]) -> dict[str, int]:
    return {
        "solver_correctness": sum(row.get("solver_correctness_denominator") == 0 for row in rows),
        ...
        "external_state": sum(row.get("external_state_coverage") == 0.0 for row in rows),
    }
